...
describe('JSON file -> object -> buffer', function() {
	var resErr;
	var allData = [], sampleData = {}, output = '', step;
	...
	before(function(finish) {
		...
		async()
			.then(function(next) {
				...
				step = 1;
				sampleData = [];
				rl.parse(fs.readFileSync(__dirname + '/data/endnote.json'))
					...
					.on('ref', function(ref) {
						if (
							(ref.isbn == '1097-685X' && ref.title == 'A method for chest drainage after pediatric cardiac surgery: a prospective randomized trial') ||
							(ref.title == 'Everolimus immunosuppression in de novo heart transplant recipients: What does the evidence tell us now?') ||
							(ref.isbn == '0012-3692' && ref.caption == '1456')
						)
							sampleData.push(ref);
						allData.push(ref);
					})
	...
	it('should extract sample data', function() {
		expect(sampleData).to.have.length(3);
	});

	it('should complete all steps', function() {
